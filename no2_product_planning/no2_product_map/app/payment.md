...
- **做法：**
    ...
    - 購買成功後觸發同步
- **排除：**
    - 伺服器端訂閱狀態驗證本身，由 CloudFunctions module 承載，本模組為其結果的客戶端消費者，見 `firebase/cloud_functions.md`
- **利弊：**
    - 客戶端狀態管理易於實作，但等待授權同步期間存在短暫延遲
    - entitlement 經 CloudFunctions 伺服器端驗證後寫入 Firestore，client 讀取判定授權，可偵測退款與撤銷
