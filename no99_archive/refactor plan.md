...
- **Impl 側：**
  ...
  - **handleReLogin 留待後續：** Spec 另定義「同 user 重登 LWW vs 不同 user 提示清資料」流程；同 user 重登已由 R10 `handlePostAuth.syncSettingsFromCloud` 自然涵蓋；不同 user 切換需另外規劃 prev_user_id 持久化、Alert prompt、本機 wipe 策略，scope 屬獨立議題（已收編入 Round 2 R5）

---

# Round 2 規劃：Logout / ReLogin / Firebase 同步生態系
