...
| R-AU-087 | 桶B乙案補寫（2026-07-17）：mock 出缺 originalTransactionId 與 transactionId 的 txn，斷言擲 invalid-argument 且 claimTxnIndex/writeEntitlement 皆未呼叫 | no3_cloud_functions functions/src/handlers/verifyTransaction.test.ts（it R-AU-087） |
| R-AU-090 | 桶B乙案補寫（2026-07-17）：pickStatus 回 undefined，斷言擲 unavailable 且 writeEntitlement 未呼叫 | no3_cloud_functions functions/src/handlers/verifyTransaction.test.ts（it R-AU-090） |
| R-AU-093 | 桶B乙案補寫（2026-07-17）：新建 handler harness，斷言以 lookupUid 歸戶的 uid 呼叫 writeEntitlement、帶 environment 與 signedDate | no3_cloud_functions functions/src/handlers/storeNotification.test.ts（it R-AU-093） |
| R-AU-094 | 桶B乙案補寫（2026-07-17）：lookupUid 回 null，斷言回 503 待 Apple 重送且 writeEntitlement/getApiClient 未呼叫 | no3_cloud_functions functions/src/handlers/storeNotification.test.ts（it R-AU-094） |
| R-AU-095 | 桶B乙案補寫（2026-07-17）：pickStatus undefined 且非 REFUND/REVOKE，斷言回 200 且 deriveEntitlement/writeEntitlement 未呼叫 | no3_cloud_functions functions/src/handlers/storeNotification.test.ts（it R-AU-095） |
...
| R-BS-004 | 桶B乙案補寫（2026-07-17）：背景備份 mock 為永不 settle 的 pending promise，斷言 signIn 照樣 resolve true 且委派恰一次 | src/contexts/AuthContext.backupNonBlocking.test.tsx（it R-BS-004） |
...
| R-CM-033 | 桶B乙案補寫（2026-07-17）：超上限名稱 createCategory 擲 ValidationError NAME_TOO_LONG 且寫入前擋下；trim 後恰上限放行對照組同測 | src/services/categoryLogic.test.ts（backstop name-length gate describe） |
...
| R-CM-070 | 桶B乙案補寫（2026-07-17）：createAccount 同款後備閘；擋下時 ensureRateForNewAccount 未呼叫、放行時恰一次 | src/services/accountLogic.test.ts（backstop name-length gate describe） |
...
| R-CU-048 | 桶B乙案補寫（2026-07-17）：鋪 USD→EUR 與 EUR→TWD 兩腿 fixture（帶已生效 date），斷言 USD→TWD 回 1 不接力、picker 對兩腿回 undefined | src/services/currencyService.test.ts（R-CU-048 describe） |
...
| R-IE-093 | 桶B乙案補寫（2026-07-17）：__getDocs.mockRejectedValue，斷言 sync resolve、走 InitialBackup 並蓋 watermark | src/services/syncEngine.test.ts（it R-IE-093） |
| R-IE-105 | 桶B乙案補寫（2026-07-17）：批次 commit 注入 resource-exhausted，斷言 sync resolve 不外拋、戳短退避、watermark 不前進、不記配額 | src/services/syncEngine.test.ts（it R-IE-105） |
| R-OF-004 | 桶B乙案補寫（2026-07-17）：同一探測拋錯注入點，斷言全量上傳批次寫恰一次、計數與 cooldown 停成功值 | src/services/syncEngine.test.ts（it R-OF-004） |
...
| R-RC-042 | 桶B乙案補寫（2026-07-17）：batch 失敗注入，斷言排程與實例整筆未套用；正向單一 batch 軟刪對照組同測 | src/services/recurringLogic.revertGuards.test.ts（it R-RC-042） |
| R-RC-045 | 桶B乙案補寫（2026-07-17）：失敗注入斷言無雙亡中間態——原紀錄未復活時新排程也未刪 | src/services/recurringLogic.revertGuards.test.ts（it R-RC-045） |
| R-RC-046 | 桶B乙案補寫（2026-07-17）：原紀錄 fixture 未軟刪，呼叫 revert 斷言不拋且紀錄維持有效 | src/services/recurringLogic.revertGuards.test.ts（it R-RC-046） |
| R-RC-050 | 桶B乙案補寫（2026-07-17）：快照實例集合輸入、queryLog 鎖不重查約定；batch 失敗 endOn/實例/新排程零套用 | src/services/recurringLogic.revertGuards.test.ts（it R-RC-050） |
...
| R-TX-081 | 桶B乙案補寫（2026-07-17）：直呼 updateTransaction 帶／不帶排程欄位各一次，斷言條件改寫與保留 | src/services/transactionLogic.updateSchedule.test.ts（it R-TX-081） |
...
| R-TX-102 | 桶B乙案補寫（2026-07-17）：updateTransfer 改到未知幣別帳戶，斷言 INVALID_CURRENCY、原 transfer 原封不動、零 rate row | src/services/transferLogic.test.ts（it R-TX-102） |

## 桶 B 可自動化待寫（31 條：小 13、中 18）
