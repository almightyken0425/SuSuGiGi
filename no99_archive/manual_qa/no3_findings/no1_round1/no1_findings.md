...
> Round 1（2026-07-11 至 2026-07-17）歸檔。內文檔名代稱與步號引用以 tag `round1-baseline` 時點的文件為準，不隨後續精煉更新。
...
- 結案：2026-07-13，採乙案規格修訂、非 app 缺陷，狀態 spec-revised
- 產品拍板：匯率生效時點跟隨來源動作的完整時點，換算取時點最新者；同日內先後由時點分勝負是產品意圖，零時歸零非產品意圖
- 拍板依據場景：今天補記三年前的轉帳，其匯率不得壓過昨天記錄的匯率；同日 14:00 與 16:00 兩筆轉帳後回頭編輯 14:00 筆，仍以 16:00 筆為現行匯率
- 曾按原規格完成 impl 修（`toUtcDayStart` 六寫入點 floor + 啟動回填 shim + guard test），拍板後全數撤除，impl 維持現行為、零改動
- 規格連動修訂：spec `no1_data_models` CurrencyRates.date 改述為含日期與時刻；`no7` 並列規則改述為同時點比更新時間，同筆轉帳未改日期的反覆編輯即同時點場景；`no8` 補錄時點改述為轉帳 `date` 值；R-DM-035、R-CU-051、R-CU-052、R-TX-095、R-TX-107、R-XD-020 規則文字同步改述；CP-A-07-01 判準改為落記錄當下時點即過、時刻粒度不另判；A-07 場次步 7、8、11 與佈置 R1 措辭同步改述
- 補判：A-07 原始證據（全庫零列落零時）依修訂後 R-DM-035 重判為過
- 已知邊界備忘：未來日轉帳的匯率立即參與取值（時點可大於當下），spec 未定義此邊界，未登缺陷
- 波及備忘：與 FINDING-03 同族同路線收案，兩欄位語意各自獨立修訂
...
## OBS-03 佔位匯率時點壓過雲端還原的歷史匯率

- 發現：2026-07-16，A 軌離線補跑佈置期（實機）
- 分類：邊角觀察，正常流程不觸發；換算規則本身運作正確

### 現象

本機空庫（硬清後）先建外幣帳戶、雲端還原後到。建帳戶時種的佔位匯率 1.0 帶當下時點，晚於還原回來的歷史真匯率（0.2、時點為原記錄日）。換算取時點最新，生效 1.0，跨幣別小計顯示 1:1。

### 判讀

- 觸發需三件齊備：本機空庫、先建同幣別帳戶、雲端還原後到；正常使用（本機已有匯率、`pairRateExists` 擋佔位）不觸發
