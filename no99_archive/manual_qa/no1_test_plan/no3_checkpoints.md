...
- no2_qa_tools 內三支工具承載三管道，各檢查點的 code block 為示意、實跑走這三支
- sqlite 定位查詢用 `no2_qa_tools/dbq.sh`
    - `no2_qa_tools/dbq.sh "<SQL>"` 動態定位 watermelon.db 後執行
    - `no2_qa_tools/dbq.sh --path` 只印路徑、`--tables` 列表
- log 擷取用 `no2_qa_tools/cdp_capture.mjs`
    - `node no2_qa_tools/cdp_capture.mjs "<marker 前綴>" <秒數>`
- Firestore 讀寫用 `no2_qa_tools/fs_admin.mjs`
    - `node no2_qa_tools/fs_admin.mjs get <docPath>`、`count <collPath>`、`query <collPath> <n>`、`set <docPath> <json>`
    - 需先放 `no2_qa_tools/serviceAccountKey.json`，已 gitignore
    - `firebase-admin` 依賴由 `no2_qa_tools/package.json` 安裝、node_modules 已 gitignore
...
- key 存 `no2_qa_tools/serviceAccountKey.json`、不進 git
...
sqlite3 "$DB" "SELECT COUNT(*) FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
sqlite3 "$DB" "SELECT launch_mode, week_start, analytics_consent FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
...
- 庫有另一 uid 活殘列（見 no4 查詢注意事項），查詢一律綁 `user_id = '<uid>'`
- db 定位指令
...
FROM settings
WHERE _status != 'deleted' AND user_id = '<uid>';
```
...
- 預期：兩筆手動列 `date` 為各自記錄當下時點，落執行當日且互不相等
- 判定：手動寫入為 Append-Only 新增且 8 筆到齊，符合即 R-CU-061 過
- 判定：主單位儲存符合即 R-DM-036 過
- 判定：手動列 `date` 落記錄當下時點即 R-DM-035 過；時刻粒度不另判
...
```bash
DB="$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)/Documents/watermelon.db"
sqlite3 "$DB" "SELECT analytics_consent FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
```

- 預期與判定
    - 該 uid 僅一列，`analytics_consent` 為 0，與畫面關閉態一致
...
```bash
DB="$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)/Documents/watermelon.db"
sqlite3 "$DB" "SELECT launch_mode FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
```
...
```bash
DB="$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)/Documents/watermelon.db"
sqlite3 "$DB" "SELECT theme FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
```
...
```bash
DB="$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)/Documents/watermelon.db"
sqlite3 "$DB" "SELECT theme, launch_mode, analytics_consent, week_start FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
```
...
APP=$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)
DB=$(find "$APP" -name 'watermelon.db' | head -1)
sqlite3 "$DB" "SELECT language FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
```
...
APP=$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)
DB=$(find "$APP" -name 'watermelon.db' | head -1)
sqlite3 "$DB" "SELECT week_start FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
```
...
APP=$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)
DB=$(find "$APP" -name 'watermelon.db' | head -1)
sqlite3 "$DB" "SELECT time_zone FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
...
- 本場排程識別：`template_amount` 對應 10021 元縮放值 `-100210000`
- 縮放倍率以 10000 計、支出取負；實庫倍率不同時以 10021 元換算值替換
...
APP=$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)
DB=$(find "$APP" -name 'watermelon.db' | head -1)
sqlite3 "$DB" "SELECT language, time_zone, week_start FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';"
sqlite3 "$DB" "SELECT start_on, end_on FROM schedules WHERE _status != 'deleted' AND deleted_on IS NULL AND template_amount = -100210000;"
sqlite3 "$DB" "SELECT COUNT(*) FROM transactions WHERE _status != 'deleted' AND deleted_on IS NULL AND schedule_id IN (SELECT id FROM schedules WHERE template_amount = -100210000);"
```
...
APP=$(xcrun simctl get_app_container booted com.almightyken0425.susugigiapp data)
DB=$(find "$APP" -name 'watermelon.db' | head -1)
sqlite3 "$DB" "SELECT t.amount, t.note, c.name, c.type, a.name, a.currency_code FROM transactions t JOIN categories c ON c.id = t.category_id JOIN accounts a ON a.id = t.account_id WHERE t._status != 'deleted' AND t.deleted_on IS NULL AND t.note IN ('Lunch','January salary') ORDER BY t.amount;"
```

- 庫有 2026-06 匯入測試殘留，`Cash` `Bank` 帳下有舊列，故以 `note` 圈定範本兩列
...
- 庫有 2026-06 殘留同名戶與類別，以 `created_at` 區分，只認本場新建列
- 預期一：本場新建的 `Cash` `Bank` 兩帳戶 `deleted_on` 非空，已軟刪
- 預期二：本場新建的 `Food` `Salary` 兩類別 `deleted_on` 非空，已軟刪
...
```bash
sqlite3 "$DB" "SELECT id, amount, _status, deleted_on FROM transactions WHERE amount = -130210000;"
```
...
```bash
sqlite3 "$DB" "SELECT amount FROM transactions WHERE amount IN (-130300000, -130310000) AND _status != 'deleted' AND deleted_on IS NULL;"
```

- 預期：活躍列金額為 `-130300000`、無 `-130310000` 活躍列
...
sqlite3 "$DB" "SELECT amount, deleted_on, _status FROM transactions WHERE amount IN (-130100000, -130110000);"
```
...
sqlite3 "$DB" "SELECT amount, deleted_on FROM transactions WHERE amount IN (-130100000, -130110000) AND _status != 'deleted' AND deleted_on IS NULL;"
```
...
sqlite3 "$DB" "SELECT amount, deleted_on FROM transactions WHERE amount IN (-130010000, -130050000);"
sqlite3 "$DB" "SELECT amount_from, deleted_on FROM transfers WHERE amount_from IN (130030000, 130040000);"
...
```bash
sqlite3 "$DB" "SELECT amount, category_id, (SELECT name FROM categories WHERE id = t.category_id) AS cat FROM transactions t WHERE amount IN (-130100000, -130110000) AND _status != 'deleted' AND deleted_on IS NULL;"
...
sqlite3 "$DB" "SELECT amount, account_id FROM transactions WHERE amount IN (-130010000, -130050000) AND _status != 'deleted' AND deleted_on IS NULL;"
sqlite3 "$DB" "SELECT amount_from, account_from_id, account_to_id, deleted_on FROM transfers WHERE amount_from IN (130030000, 130040000);"
...
- 預期三：13003 轉帳因合併後將成自轉帳而軟刪、`deleted_on` 非空；實作軟刪不改指，`account_from_id` 保留來源
...
- 交易匯入不去重，重匯必再翻一份，非全略過語意
- 摘要應顯示共匯入 6、略過 0（與步 22 那輪相同，非累加去重）
- 存活列數應為步 22 存活數加本輪匯入數（步 22 若已 6 筆存活，本步應為 12）
- 存活列數與此推算一致即符合 R-KR-005，重點驗殺於成功對話框後資料不掉、不留半套
...
```sql
SELECT
 (SELECT count(*) FROM accounts       WHERE _status!='deleted')
+(SELECT count(*) FROM categories     WHERE _status!='deleted')
+(SELECT count(*) FROM transactions   WHERE _status!='deleted')
+(SELECT count(*) FROM transfers      WHERE _status!='deleted')
+(SELECT count(*) FROM currency_rates WHERE _status!='deleted')
+(SELECT count(*) FROM schedules      WHERE _status!='deleted') AS total;
```

- `Total Pushed` 的 N 等於本機六集合列總數：R-IE-100 過、R-AU-079 過
- 列總數含軟刪墓碑列；全量備份連墓碑一併上傳、供還原時傳播刪除狀態
...
- Found 的 M 僅涵蓋 lastSyncedAt 之後變更的六集合列：R-IE-102 過
- M 為 6，本輪手建 4 筆加上跨幣別轉帳自動補錄的匯率正反 2 筆
...
- 2026-07-14 實測 600 筆備份兩毫秒完成，兩觸發不重疊、live 無法重現重入
- 單飛閘為同步 check 加 set、回同一個 promise，程式碼面判定正確；本條改以 syncEngine 單元測試代測，live 抓不到不算 FAIL
...
CACHE="$CONTAINER/Library/Application Support/com.almightyken0425.susugigiapp/RCTAsyncLocalStorage_V1/manifest.json"
python3 - "$CACHE" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
for k, v in d.items():
...
- 回應 error status 為 UNAUTHENTICATED 即未登入拒絕請求，R-AU-085 過
- 後端 enforceAppCheck 先於身分檢查，curl 無 App Check token 時回泛用 Unauthenticated、非登入守門的專用訊息；拒絕語意在更外層被滿足
...
- 回應為 error 非成功 tier，且 entitlements 文件未因本次呼叫變動
- enforceAppCheck 使 curl 到不了簽章驗證層，偽造簽章的專屬拒絕路徑外部不可觀測
- 簽章拒絕改以後端 verifyTransaction 單元測試代測；curl 面僅能確認拒絕與授權未變動
...
SELECT last_synced_at FROM settings WHERE _status != 'deleted' AND user_id = '<uid>';
```
...
- 執行註記 2026-07-16
    ...
    - R-OF-009 實測不降級，登 round1 缺陷登記 FINDING-13
